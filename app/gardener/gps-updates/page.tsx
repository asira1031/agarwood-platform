...
export default function GardenerGpsUpdatesPage() {
  ...
  const [gpsLogs, setGpsLogs] = useState<Row[]>([]);
  ...
  async function loadData() {
    ...
    let caretakerQuery = supabase
      .from("caretakers")
      .select("*")
      .eq("caretaker_profile_id", profileRow.id)
      .maybeSingle();

    let { data: caretakerRow, error: caretakerError } = await caretakerQuery;
  ...
  function verifyTreeCode() {
    ...
    if (input === treeId || input === treeCode) {
      ...
      setMessage("Tree Verified. You may submit GPS evidence.");
      return;
    }
  ...
  async function submitGpsEvidence() {
    setMessage("");
    ...
    if (!latitude.trim() || !longitude.trim()) {
      setMessage("GPS latitude and longitude are required.");
      return;
    }
    ...
    try {
      ...
      const { error: gpsError } = await supabase.from("tree_gps_logs").insert({
      ...
      if (photoUrl) {
        const { error: photoError } = await supabase.from("tree_photo_updates").insert({
          ...
          notes: notes.trim() || "Optional GPS verification photo",
          status: "SUBMITTED",
          created_at: now,
          updated_at: now,
        });
      ...
      setTreeVerified(false);
      setScannedCode("");
      setMessage("GPS evidence submitted for admin review.");
      await loadData();
    } catch (error: any) {
      setMessage(error.message || "GPS evidence submission failed.");
    } finally {
      setSaving(false);
    }
  }
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        <header className="rounded-3xl border border-white/10 bg-[#071f16]/90 p-8 shadow-2xl">
          ...
          <p className="mt-2 text-white/70">
            Verify the assigned tree first, then submit GPS evidence for admin review.
          </p>
        </header>
        ...
        {loading ? (
          <div className="rounded-3xl border border-white/10 bg-white/10 p-8 text-white/70">
            Loading GPS workflow...
          </div>
        ) : assignments.length === 0 ? (
          <div className="rounded-3xl border border-white/10 bg-white/10 p-8 text-white/70">
            No active GPS assignments yet.
          </div>
        ) : (
          <section className="grid gap-6 lg:grid-cols-5">
            ...
            <div className="space-y-6 lg:col-span-4">
              ...
              <div className="rounded-3xl border border-white/10 bg-white/[0.06] p-6">
                ...
                <p className="mt-2 text-sm text-white/60">
                  GPS required. Photo and notes are optional.
                </p>

                <button
                  onClick={useCurrentLocation}
                  className="mt-4 w-full rounded-xl border border-[#d9b45f]/30 bg-[#d9b45f]/10 px-5 py-3 font-black text-[#f7d774]"
                >
                ...
                <textarea
                  ...
                  className="mt-2 min-h-[120px] w-full rounded-xl border border-white/10 bg-white/10 px-4 py-3 text-white outline-none"
                  placeholder="Optional GPS notes..."
                />
              </div>

              <button
                onClick={submitGpsEvidence}
              ...
              <div className="rounded-3xl border border-white/10 bg-white/[0.06] p-6">
                <h2 className="text-2xl font-black text-[#ffe49a]">Recent GPS Logs</h2>
                <div className="mt-4 space-y-3">
                  {gpsLogs.slice(0, 10).map((item) => (
                    <div key={item.id} className="rounded-2xl bg-black/20 p-4">
                      ...
                      {item.notes && (
                        <p className="mt-2 text-sm text-white/70">{item.notes}</p>
                      )}
                    </div>
                  ))}
                </div>
